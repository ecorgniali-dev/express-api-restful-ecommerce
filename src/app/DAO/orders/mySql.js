...
class MySQLDao extends IDao {
    ...
    async create(cliente, itemsClientCart) {
        let newOrderData = await knex(this.tableName)
            .insert({ email: cliente.email, direccion: cliente.direccion });
        ...
        let newOrderItemsData = await knex(this.tableNameItems)
            .insert(orderItems);
    ...
    async read(query) {
        let ordersDB = await knex(this.tableName)
            .select('*')
            .where(query);

        let ordersItemsDB = await knex(this.tableNameItems)
            .join('productos', 'product_id', '=', 'productos.id')
            .select('productos.codigo', 'productos.nombre', 'productos.descripcion', 'productos.foto', 'ordenes_items.precio', 'ordenes_items.cantidad', 'ordenes_items.order_id');
